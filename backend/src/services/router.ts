...
import { Dex, Quote } from './dex/base.js'; // Import Quote
...
class DexRouter {
  ...
  async findBestRoute(tokenIn: string, tokenOut: string, amount: number) {
    ...
    const validQuotes = rawQuotes.filter((q): q is { dex: Dex; quote: Quote } =>
      q !== null && q.quote && typeof q.quote.price === 'number' && typeof q.quote.fee === 'number' && !isNaN(q.quote.price) && !isNaN(q.quote.fee)
    );
